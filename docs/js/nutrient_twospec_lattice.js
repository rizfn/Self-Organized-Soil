...
data.forEach((d) => {
    ...
    const fives = lattice.reduce((a, b) => a + b.filter((x) => x === 5).length, 0);
	d.vacancy = zeros / L**2;
	d.nutrient = (ones + twos) / L**2;
	d.soil = threes / L**2;
    d.worm = (fours + fives) / L**2;
...
var rows = d3.map(data, function(d){return d.theta2;})
...
var Tooltip = d3.select("div#raster")
	...
	Tooltip
		.html("d=" + d3.format("0.2f")(d.theta2) + ", " + "s=" + d3.format("0.2f")(d.sigma) + "<br>" + d3.format("0.4f")(d.soil_boundaries))
...
let current_soil_lattice_state = {"theta2": 0, "sigma": 0};

// create a heatmap on mouseclick
var mousedown = function(event, d) {
	current_soil_lattice_state = {"theta2": d.theta2, "sigma": d.sigma};
...
var svg_soil = d3.select("div#raster")
  ...
  .append("g")
	...
	Tooltip
        .html("d=" + d3.format("0.2f")(d.theta2) + ", " + "s=" + d3.format("0.2f")(d.sigma) + "<br>" + d3.format("0.2")(d.soil) + ", " + d3.format("0.2")(d.vacancy) + ", " + d3.format("0.2")(d.nutrient) + ", " + d3.format("0.2")(d.worm))
...
function refilter_data() {
	...
	const soil_lattice = filtereddata.filter(function(d) {return d.theta2 == current_soil_lattice_state.theta2 && d.sigma == current_soil_lattice_state.sigma})[0].soil_lattice
	// update the lattice
