...
void run_timeseries(int STEPS_PER_LATTICEPOINT, int L, double sigma, double theta, double rho1, double mu1, std::ofstream &file)
{
    ...
    for (int step = 0; step <= STEPS_PER_LATTICEPOINT; ++step)
    {
        for (int i = 0; i < L * L; ++i)
        {
            update(soil_lattice, L, sigma, theta, rho1, mu1, &soil_production, &empty_production, &nutrient_production, &worm_production);
        }
        int counts[4] = {0};
...
int main(int argc, char *argv[])
{
    ...
    filePathStream << exeDir << "\\outputs\\timeseriesWormCounts2DProd\\sigma_" << sigma << "_theta_" << theta << ".csv";
    std::string filePath = filePathStream.str();

    std::ofstream file;
    file.open(filePath);
    file << "step,emptys,nutrients,soil,greens,empty_production,nutrient_production,soil_production,worm_production\n";
