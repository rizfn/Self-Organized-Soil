...
import pandas as pd
from two_species_soil_neighbour_utils import run_stochastic_asym


def run_raster_double_d(n_steps, L, r, d_list, s_list, steps_to_record=np.array([100, 1000, 10000, 100000])):
    """Run the rasterscan where the blue bacteria has twice the death rate for n_steps timesteps.
    ...
    for i in tqdm(range(len(ds_pairs))):  # todo: parallelize
        d, s = ds_pairs[i]
        soil_lattice_data = run_stochastic_asym(n_steps, L, r, r, d, 2*d, s, s, steps_to_record=steps_to_record)
...
def main():
    ...
    d = np.linspace(0, 0.1, 20)  # death rate
    s = np.linspace(0, 0.8, 20)  # soil filling rate

    # NEIGHBOURS
    soil_lattice_data = run_raster_double_d(n_steps, L, r, d, s, np.geomspace(1000, n_steps, int(np.log10(n_steps/1000))+1, dtype=np.int32))
    soil_lattice_data = pd.DataFrame(soil_lattice_data)
    soil_lattice_data.to_json(f"docs/data/two_species/double_d_{r=}.json", orient="records")
