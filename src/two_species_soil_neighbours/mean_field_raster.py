...
import pandas as pd
from two_species_soil_neighbour_utils import ode_integrate
...
def run_raster(n_steps, r, d_list, s_list, steps_to_record=np.array([100, 1000, 10000, 100000])):
    ...
    for i in tqdm(range(len(ds_pairs))):  # todo: parallelize
        d, s = ds_pairs[i]
        T, S, E_R, E_B, R, B = ode_integrate(s, d, r, stoptime=n_steps, nsteps=100_000)
        for step in steps_to_record:
            soil_lattice_list.append({"d": d, "s": s, "step":step, "redvacancy": E_R[step], "bluevacancy": E_B[step], "red":R[step], "blue": B[step], "soil": S[step]})
...
def main():
    ...
    # save the data
    soil_lattice_data.to_json(f"docs/data/two_species/soil_neighbours_meanfield_{r=}.json", orient="records")
