...
__global__ void updateKernel(bool *d_lattice, curandState *state, int offsetX, int offsetY)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int idy = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    else // if the site is empty
    {
        ...
        int nbrX = (siteX + curand(&localState) % 3 - 1 + L) % L;
        int nbrY = (siteY + curand(&localState) % 3 - 1 + L) % L;
