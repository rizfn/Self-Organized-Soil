...
def load_csv(filename):
    ...
    with open(filename, 'r') as f:
        ...
        for row in reader:
            ...
            filled_cluster_sizes.append([int(x) for x in row[1].split(',')])  # Convert to int and add to filled_cluster_sizes
            empty_cluster_sizes.append([int(x) for x in row[2].split(',')])  # Convert to int and add to empty_cluster_sizes
    return steps, filled_cluster_sizes, empty_cluster_sizes

def main():
    csv_files = glob.glob('src/cuda_test/simpleNbrGrowth/outputs/csd/*.tsv')
    ...
    plt.tight_layout()
    plt.savefig('src/cuda_test/simpleNbrGrowth/plots/csd/csd_powerlaw.png', dpi=300)
    plt.show()


def main_3D():
    csv_files = glob.glob('src/cuda_test/simpleNbrGrowth/outputs/csd_3D/*.tsv')
    num_files = len(csv_files)
    ...
    plt.tight_layout()
    plt.savefig('src/cuda_test/simpleNbrGrowth/plots/csd/csd_3D_powerlaw.png', dpi=300)
    plt.show()
