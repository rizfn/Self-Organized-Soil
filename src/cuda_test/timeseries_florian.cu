...
__global__ void updateKernel(bool *d_lattice, curandState *state, bool isB)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int idy = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    if ((blockIdx.x + blockIdx.y) % 2 == isB)
    {
...
void run(std::ofstream &file)
{
    ...
    for (int step = 0; step < N_STEPS; ++step)
    {
        ...
        {
            // Update 'A' cells
            updateKernel<<<gridSize, blockSize>>>(d_lattice, d_state, false);
            cudaDeviceSynchronize();

            // Update 'B' cells
            updateKernel<<<gridSize, blockSize>>>(d_lattice, d_state, true);
            cudaDeviceSynchronize();
        }
