...
import pandas as pd
from soil_neighbour_utils import predator_prey_ode_integrate


def run_predator_prey_meanfield_raster(n_steps, r, d_list, s_list, steps_to_record=np.array([100, 1000, 10000, 100000])):
    ...
    for i in tqdm(range(len(ds_pairs))):  # todo: parallelize
        d, s = ds_pairs[i]
        T, S, E, B = predator_prey_ode_integrate(s, d, r, stoptime=n_steps, nsteps=100_000)
...
def main():
    ...
    soil_lattice_data = run_predator_prey_meanfield_raster(n_steps, r, d, s)

    soil_lattice_data = pd.DataFrame(soil_lattice_data)

    # save the data
    soil_lattice_data.to_json(f"docs/data/single_species/predatorprey_meanfield_{r=}.json", orient="records")
