...
// Define constants
constexpr std::array<double, 6> p_values = {0.145, 0.146, 0.147, 0.199, 0.2, 0.201};
constexpr int L = 100;
...
int main(int argc, char *argv[])
{
    ...
    for (double p : p_values)
    {
        threads.push_back(std::thread([p, &argv]() { // Capture p by value
            ...
            filename << exeDir << "/outputs/CSD3D/criticalPointsCPUBCC/p_" << p << "_L_" << L << ".tsv";
