...
// Define constants
constexpr std::array<double, 6> p_values = {0.03167, 0.0317, 0.03172, 0.108, 0.11, 0.112};
constexpr int L = 16;
...
std::vector<std::vector<std::vector<std::vector<std::vector<bool>>>>> initLattice()
{
    std::vector<std::vector<std::vector<std::vector<std::vector<bool>>>>> soil_lattice(L, std::vector<std::vector<std::vector<std::vector<bool>>>>(L, std::vector<std::vector<std::vector<bool>>>(L, std::vector<std::vector<bool>>(L, std::vector<bool>(L, false)))));
        ...
        for (int j = 0; j < L; ++j)
        {
            for (int k = 0; k < L; ++k)
            {
                for (int m = 0; m < L; ++m)
                {
                    for (int n = 0; n < L; ++n)
                    {
                        if ((i + j + k + m + n) % 2 == 1) // Set odd cells to true
                        {
                            soil_lattice[i][j][k][m][n] = dis(gen);
                        }
                    }
                }
            }
...
void updateLattice(std::vector<std::vector<std::vector<std::vector<std::vector<bool>>>>> &lattice, double p)
{
    std::vector<std::vector<std::vector<std::vector<std::vector<bool>>>>> newLattice(L, std::vector<std::vector<std::vector<std::vector<bool>>>>(L, std::vector<std::vector<std::vector<bool>>>(L, std::vector<std::vector<bool>>(L, std::vector<bool>(L, false)))));
...
std::pair<std::vector<int>, std::vector<int>> get_cluster_sizes(const std::vector<std::vector<std::vector<std::vector<std::vector<bool>>>>> &lattice)
{
    UnionFind uf_filled(L * L * L * L * L);
    UnionFind uf_empty(L * L * L * L * L);
    for (int i = 0; i < L; ++i)
    {
        for (int j = 0; j < L; ++j)
        {
            for (int k = 0; k < L; ++k)
            {
                for (int m = 0; m < L; ++m)
                {
                    for (int n = 0; n < L; ++n)
                    {
                        int index = (((i * L + j) * L + k) * L + m) * L + n;
                        if (lattice[i][j][k][m][n])
                        {
                            for (int dir = 0; dir < 32; ++dir)
                            {
                                int x = i + (dir % 2);
                                ...
                                if (lattice[x][y][z][w][v])
                                {
                                    int neighbor_index = (((x * L + y) * L + z) * L + w) * L + v;
...
{
    std::vector<std::vector<std::vector<std::vector<std::vector<bool>>>>> lattice = initLattice();
...
int main(int argc, char *argv[])
{
    ...
    for (double p : p_values)
    {
        threads.push_back(std::thread([p, &argv]() { // Capture p by value
            ...
            filename << exeDir << "/outputs/CSD5D/criticalPointsCPUBCC/p_" << p << "_L_" << L << ".tsv";
