...
{
    std::vector<bool> newLattice = lattice;
    for (int site = 0; site < L; ++site)
    {
        if (lattice[site]) // if the site is active
        {
            ...
            {newLattice[site] = false;}
            if (dis_prob(gen) < P)
            {newLattice[left] = true;}
            else
            {newLattice[left] = false;}
            if (dis_prob(gen) < P)
            {newLattice[right] = true;}
            else
            {newLattice[right] = false;}
        }
    }
    lattice = newLattice;
}
...
int main(int argc, char *argv[])
{
    ...
    filename << exeDir << "/outputs/timeseries1D/overwrite_p_" << P << ".tsv";
