...
void updateEatAll(std::vector<std::vector<int>> &soil_lattice, int L, double sigma, double theta, double rho, double mu)
{
...
void run_timeseries(int N_STEPS, int L, double sigma, double theta, double rho, double mu, std::ofstream &file)
{
    ...
    for (int step = 0; step <= N_STEPS; ++step)
    {
        ...
        if (step % (L * L) == 0) // Changed to 2D
        {
            ...
            for (const auto &row : soil_lattice) // Changed to 2D loop
            {
                for (int cell : row)
                {
                    ++counts[cell];
            ...
            file << step / (L * L); // Changed to 2D
            for (int j = 0; j < counts.size(); ++j)
            {
                file << "," << static_cast<double>(counts[j]) / (L * L); // Changed to 2D
            }
            file << "\n";
...
int main()
{
    ...
    filePathStream << exeDir << "\\..\\outputs\\timeseries2D\\Nspec\\" << N << "spec_sigma_" << SIGMA << "_theta_" << THETA << ".csv"; // Changed to 2D
