...
constexpr int NUTRIENT_GREEN = 1;
constexpr int NUTRIENT_BLUE = 2;
constexpr int NUTRIENT_PURPLE = 3;
constexpr int SOIL = 4;
constexpr int GREEN_WORM = 5;
constexpr int BLUE_WORM = 6;
constexpr int PURPLE_WORM = 7;

constexpr std::array<int, N> NUTRIENTS = {NUTRIENT_GREEN, NUTRIENT_BLUE, NUTRIENT_PURPLE};
constexpr std::array<int, N> WORMS = {GREEN_WORM, BLUE_WORM, PURPLE_WORM};
...
void run_timeseries(int N_STEPS, int L, double sigma, double theta, double rho, double mu, std::ofstream& file)
{
    ...
    for (int step = 0; step <= N_STEPS; ++step)
    {
        ...
        if (step % (L * L * L) == 0)
        {
            std::vector<int> counts(8, 0); // 8 different possible states
...
int main(int argc, char *argv[])
{
    ...
    filePathStream << exeDir << "\\..\\outputs\\timeseries3D\\3spec\\sigma_" << sigma << "_theta_" << theta << "_rho_" << rho << "_mu_" << mu << ".csv";
    std::string filePath = filePathStream.str();
    
    std::ofstream file;
    file.open(filePath);
    file << "step,emptys,nutrient_green,nutrient_blue,nutrient_purple,soil,greens,blues,purples\n";
