...
constexpr int FINAL_STEPS_TO_RECORD = 400;
...
void run(long long N_STEPS, int L, double sigma, double theta, double rho, double mu, std::ofstream &file)
{
    ...
    long long recordingStep = N_STEPS - (FINAL_STEPS_TO_RECORD * L * L * L);
...
int main()
{
    ...
    filePathStream << exeDir << "\\..\\..\\..\\docs\\data\\multispec_nutrient\\" << N << "spec\\sigma_" << SIGMA << "_theta_" << THETA << ".tsv";
