    ...
    for (int i = 0; i < L; ++i)
    {
        for (int j = 0; j < L; ++j)
        {
            ...
            if (pattern_index < N)
            {
                ...
                soil_lattice[i][j] = WORMS[pattern_index];
            }
            else
            {
                // Assign soil
                soil_lattice[i][j] = SOIL;
            }
        }
    }
    return soil_lattice;
...
int main(int argc, char *argv[])
{
    ...
    filePathStream << exeDir << "/../outputs/confinement/algo_comparison/other2/cpp_" << time(0) << ".tsv";
