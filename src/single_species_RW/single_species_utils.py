...
def update_wellmixed(soil_lattice, L, r, d, s):
    ...
    for site in bacteria_sites:
        ...
        if new_site_value == 1:
            ...
            random_spawnpoints = np.array([[np.random.randint(0, L), np.random.randint(0, L)], [np.random.randint(0, L), np.random.randint(0, L)], [np.random.randint(0, L), np.random.randint(0, L)], [np.random.randint(0, L), np.random.randint(0, L)]])
            for rdm_spawnpoint in random_spawnpoints:
                if soil_lattice[rdm_spawnpoint[0], rdm_spawnpoint[1]] == 0:
                    if np.random.rand() < r:
                        soil_lattice[rdm_spawnpoint[0], rdm_spawnpoint[1]] = 2
                        break
