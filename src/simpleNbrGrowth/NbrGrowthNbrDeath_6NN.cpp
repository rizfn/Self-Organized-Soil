...
Coordinate get_random_neighbour(Coordinate site, int L) {
    int dir = dis_dir(gen) % 6; // change 8 to 6 for hexagonal grid
    bool isEvenColumn = site.x % 2 == 0;
    switch (dir) {
        case 0: // left
        ...
        case 2: // top-right
            return {(site.x + 1) % L, (site.y - (isEvenColumn ? 1 : 0) + L) % L};
        case 3: // bottom-right
            return {(site.x + 1) % L, (site.y + (isEvenColumn ? 0 : 1)) % L};
        case 4: // bottom-left
            return {(site.x - 1 + L) % L, (site.y + (isEvenColumn ? 0 : 1)) % L};
        case 5: // top-left
            return {(site.x - 1 + L) % L, (site.y - (isEvenColumn ? 1 : 0) + L) % L};
    }
...
std::pair<std::vector<int>, std::vector<int>> get_cluster_sizes(const std::vector<bool>& lattice, int L) {
    ...
    for (int i = 0; i < L; ++i) {
        for (int j = 0; j < L; ++j) {
            ...
            if (lattice[i * L + j]) {
                if (j > 0 && lattice[i * L + j - 1]) // left
                    uf_filled.union_set(i * L + j, i * L + j - 1);
                if (j < L - 1 && lattice[i * L + j + 1]) // right
                    uf_filled.union_set(i * L + j, i * L + j + 1);
                if (i > 0 && lattice[(i - 1) * L + (j + (isEvenColumn ? 0 : 1)) % L]) // top-right
                    uf_filled.union_set(i * L + j, (i - 1) * L + (j + (isEvenColumn ? 0 : 1)) % L);
                if (i < L - 1 && lattice[(i + 1) * L + (j + (isEvenColumn ? 0 : 1)) % L]) // bottom-right
                    uf_filled.union_set(i * L + j, (i + 1) * L + (j + (isEvenColumn ? 0 : 1)) % L);
                if (i < L - 1 && lattice[(i + 1) * L + (j - (isEvenColumn ? 1 : 0) + L) % L]) // bottom-left
                    uf_filled.union_set(i * L + j, (i + 1) * L + (j - (isEvenColumn ? 1 : 0) + L) % L);
                if (i > 0 && lattice[(i - 1) * L + (j - (isEvenColumn ? 1 : 0) + L) % L]) // top-left
                    uf_filled.union_set(i * L + j, (i - 1) * L + (j - (isEvenColumn ? 1 : 0) + L) % L);
            } else {
                if (j > 0 && !lattice[i * L + j - 1]) // left
                ...
                if (j < L - 1 && !lattice[i * L + j + 1]) // right
                    uf_empty.union_set(i * L + j, i * L + j + 1);
                if (i > 0 && !lattice[(i - 1) * L + (j + (isEvenColumn ? 0 : 1)) % L]) // top-right
                    uf_empty.union_set(i * L + j, (i - 1) * L + (j + (isEvenColumn ? 0 : 1)) % L);
                if (i < L - 1 && !lattice[(i + 1) * L + (j + (isEvenColumn ? 0 : 1)) % L]) // bottom-right
                    uf_empty.union_set(i * L + j, (i + 1) * L + (j + (isEvenColumn ? 0 : 1)) % L);
                if (i < L - 1 && !lattice[(i + 1) * L + (j - (isEvenColumn ? 1 : 0) + L) % L]) // bottom-left
                    uf_empty.union_set(i * L + j, (i + 1) * L + (j - (isEvenColumn ? 1 : 0) + L) % L);
                if (i > 0 && !lattice[(i - 1) * L + (j - (isEvenColumn ? 1 : 0) + L) % L]) // top-left
                    uf_empty.union_set(i * L + j, (i - 1) * L + (j - (isEvenColumn ? 1 : 0) + L) % L);
            }
        }
...
int main(int argc, char *argv[])
{
    ...
    filename << exeDir << "/outputs/csdNbrDeath/6NN_sigma_" << SIGMA << "_theta_" << THETA << ".tsv";
